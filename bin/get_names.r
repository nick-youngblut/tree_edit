...
option_list <- list(
	...
	make_option(c("-z", "--description"), action="store_false", default=TRUE, help="Write out names as orderd in the phylogeny")
	)
...
if(is.null(opt$outname)){ opt$outname <- ext.edit(opt$tree, paste(c("_name", ".txt"), collapse="")) }


### data processing
if(ext == ".nwk"){ tree <- read.tree(opt$tree) } else
if(ext == ".tre"){ tree <- read.nexus(opt$tree) }
name.tbl <- as.data.frame(tree$tip)
write.table(name.tbl, opt$outname, quote=FALSE, sep="", row.names=FALSE, col.names=FALSE)
